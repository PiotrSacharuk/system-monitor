...
DiskSensor::DiskSensor(double alertThreshold)
    : Sensor("DISK", alertThreshold) {}
...
void DiskSensor::fetchData() {
    ...
    while (std::getline(diskStats, line)) {
        if (line.find("sda ") != std::string::npos) {
            ...
            sscanf(line.c_str(), "%u %u %*s %llu %*u %*u %*u %*u %llu", &major,
                   &minor, &reads, &writes);
            break;
        }
    }
    currentValue = std::min(95.0, 40.0 + (reads % 100) / 2.0);
    std::cout << "Disk data collected from /proc/diskstats ("
